...
file = open("kamala_new.txt")
line = {}
dictionary = {}
#These are the words to not include from the file
words_remove = ("a", "\n", "", "and", "the", "to", "of", "i", "in", "for", "our", "we", "that", "is", "he", "are", "who", "my", "with", "will", "us", "be", "as", "she", "not", "you", "on", "it", "an", "when", "has", "but", "this", "have", "was", "would", "their", "me", "they", "his", "about", "know", "at", "because", "out", "what", "so", "from", "your", "or")
for line in file:
...
file = open("kamala_new.csv", "w")
for key, value in dictionary.items():
        if value > 3:          
            file.write(key + ',' + str(value)+'\n')
file.close() 
#This adds a header for Plotly to use
df = pd.read_csv("kamala_new.csv", sep=',', header=None)
df.to_csv("kamala_new.csv", header=['words', 'count'], index=False)
df = pd.read_csv('kamala_new.csv')
#This uses Plotly to make a bar graph of the instances of the words
fig = px.bar(df, x = 'words', y = 'count', title="Words Kamala used in speech")
fig.show()
